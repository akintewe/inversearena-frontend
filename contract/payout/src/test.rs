...
fn test_zero_amount_panics() {
    ...
    let result = client.try_distribute_winnings(&admin, &1u32, &winner, &amount, &currency);
...
fn test_idempotency_prevents_double_pay_same_amount() {
    ...
    let second_attempt = client.try_distribute_winnings(&admin, &1u32, &winner, &amount, &currency);
...
fn test_idempotency_prevents_double_pay_different_amount() {
    ...
    client.distribute_winnings(&admin, &99u32, &winner, &first_amount, &currency);

    let second_attempt =
        client.try_distribute_winnings(&admin, &99u32, &winner, &second_amount, &currency);
    assert_eq!(second_attempt, Err(Ok(PayoutError::AlreadyPaid)));
